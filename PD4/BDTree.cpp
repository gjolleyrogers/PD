...
int Usage(int argc, char** argv)
{
    ...
    std::cerr << argv[0] << "  N_taxa_Current    speciation_rate extinction_rate   "  << std::endl;;
...
int main(int argc, char** argv)
{
    ...
    timeb tb, te;
    int NumLeaves = 100;
    double speciation_rate = 0.8;
    double extinction_rate = 0.4;
    ...
    if (   argc> 3   )  try
            {
                extinction_rate = std::stod(  argv[3]) ;
            }
        catch( std::invalid_argument )
            {
                return Usage(argc,argv);
            }
    if ( speciation_rate <= extinction_rate ) return Usage(argc,argv);
    ...
    Phyla = new( Phylogeny);
    Phyla->BDTree(NumLeaves, speciation_rate, extinction_rate );
