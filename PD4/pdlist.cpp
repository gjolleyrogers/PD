...
int Usage(int argc, char** argv)
{
    ...
    std::cerr << argv[0] << "  file1    file2  " << std::endl << std::endl;;
    std::cerr <<  "  file1 contains a phylogeny in newick form    " << std::endl;
    std::cerr <<  "  file2   \"     the  list of taxa to be marked for calc of PD" << std::endl;
    std::cerr <<  "  and additional files containing lists e.g   " << std::endl;
    return 1;
...
int main(int argc, char** argv)
{
    ...
    bool verbose = true;
    PD p;
    p.calc_pdLists(argc, argv);  //calculate the PD of the phylogeny and any sublists
